...
| 3  | 集計マートの構築と可視化                 | BigQuery SQL, Looker Studio           | 完了 |
...
- 期間: 2010-12-01 〜 2011-02-27（67日分）
- 主なデータ品質の特徴: `CustomerID` 約34%欠損、キャンセル取引（`InvoiceNo` が C 始まり）が混在
...
│
├── 03_mart/                    # Part 3: 集計マートと可視化
│   └── sql/
│       ├── daily_summary.sql       # v_daily_summary ビュー（日別×国別）
│       └── weekday_pattern.sql     # v_weekday_pattern ビュー（曜日別）
│
...
- [03_mart/README.md](03_mart/README.md) — 集計ビュー作成・動作確認・Looker Studio 接続
